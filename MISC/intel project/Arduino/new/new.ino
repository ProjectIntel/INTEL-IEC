...
void loop() 
{
  for(leftRightPos = 0; leftRightPos < 180; leftRightPos+=1)    // going left to right.
 {
    leftRightServo.write(leftRightPos);
      for (inde = 0; inde<=numReadings;inde++) {            // take x number of readings from the sensor and average them
    ...
    if (inde >= numReadings)  {                               // reset the counts when at the last item of the array
      inde = 0;
      total = 0;
    }
  ...
  */
  for(leftRightPos = 180; leftRightPos >=1; leftRightPos-=1)   // going right to left
   {
    leftRightServo.write(leftRightPos);
    for (inde = 0; inde<=numReadings;inde++) {
      digitalWrite(trigPin, LOW);
      delayMicroseconds(2);
